...
| 字段 | 类型 | 含义 | 约束 |
| --- | --- | --- | --- |
| `axle_positions_mm` | 整数数组 | 各轴位置，毫米，**按车头方向严格递增**（第 1 项为车尾轴，末项为车头轴） | 必填；轴数 1～12；每项 0～100000 且不重复 |
| `axle_loads_kg` | 整数数组 | 与轴位置一一对应的轴载荷，千克 | 必填；项数与轴位置一致；每项 1～20000 |
| `bridge_length_mm` | 整数 | 桥面有效长度，毫米 | 必填；1000～50000 |
| `approved_load_kg` | 整数 | 现场核定载荷，千克 | 必填；1～200000 |

任一约束不满足（含字段缺失、位置重复或未递增、数值越界——含极大轴位置或
载荷）都**统一返回 HTTP 422**，响应体只有错误信封 `{"error": ...}`，
**不会生成任何分析结果**。数值上限同时保证全部整数运算不溢出：
位移最大 100000+50000 毫米，落桥载荷合计最大 12×20000 千克。
...
| 桥面窗口 | `POST /api/v1/bridge-window`：轴位置按车头方向严格递增且不重复（1～12 轴、位置 0～100000mm、载荷 1～20000kg），桥长 1000～50000mm，核定载荷 1～200000kg；扫描全部进入/离开事件，返回最大桥面载荷、首尾轴、发生位移与 `通行`/`拦停` |
