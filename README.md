...
| 字段 | 类型 | 含义 | 约束 |
| --- | --- | --- | --- |
| `axle_positions_mm` | 整数数组 | 各轴位置，毫米，**按车头方向严格递增**（第 1 项为车尾轴，末项为车头轴） | 必填；轴数 1～12；每项 ≥ 0 且不重复 |
| `axle_loads_kg` | 整数数组 | 与轴位置一一对应的轴载荷，千克 | 必填；项数与轴位置一致；每项 ≥ 1 |
...
| 桥面窗口 | `POST /api/v1/bridge-window`：轴位置按车头方向严格递增且不重复（1～12 轴、位置 ≥ 0），桥长 1000～50000mm，核定载荷 1～200000kg；扫描全部进入/离开事件，返回最大桥面载荷、首尾轴、发生位移与 `通行`/`拦停` |
