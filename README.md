...
| 字段 | 类型 | 含义 | 约束 |
| --- | --- | --- | --- |
| `axle_positions_mm` | 整数数组 | 各轴位置，毫米，**按车头方向严格递增**（第 1 项为车尾轴，末项为车头轴） | 必填；轴数 1～12；每项必须是**非 null 整数**、≥ 0 且不重复，**不设上限（可超出 int64）**；数组元素填 `null` 一律 422，不会被当作 0 |
| `axle_loads_kg` | 整数数组 | 与轴位置一一对应的轴载荷，千克 | 必填；项数与轴位置一致；每项必须是**非 null 整数**且 ≥ 1，**不设上限（可超出 int64）**；数组元素填 `null` 一律 422 |
| `bridge_length_mm` | 整数 | 桥面有效长度，毫米 | 必填；1000～50000 |
| `approved_load_kg` | 整数 | 现场核定载荷，千克 | 必填；1～200000 |
| `speed_mm_per_s` | 整数 | **选填**：车辆匀速通过桥面的预计车速，毫米每秒；提供时额外返回超载区段与累计暴露时长 | 缺省或 `null` 时不计算、响应与引入车速能力前**逐字节一致**；提供时必须为 **1～50000** 的整数（0、负数、越界、非整数或类型错误均 422） |

任一约束不满足（含字段缺失、数组元素填 `null`、位置重复或未递增、数值越界、
预计车速为零或越界）都**统一返回 HTTP 422**，
响应体只有错误信封 `{"error": ...}`，**不会生成任何分析结果**（含车速产物）。
数组中的 `null` 元素不会被静默当作 `0`：错误会点名是哪个字段的第几项缺失。
轴位置与轴载荷不设上限：二者连同事件位移、落桥载荷合计全部按
**任意精度整数**计算与排序，超出 int64 范围的极大数值不会溢出，
最大桥面载荷与取得峰值时的车辆位移都不会被报成零、负数或少算，
`max_load_kg`、`displacement_mm` 始终作为 JSON 数字精确返回。
...
7. **预计车速与超载区段（选填）**：提供 `speed_mm_per_s` 时，复用上述轴进入、离开
   事件，在**相邻事件位移之间**切分恒定载荷区段：两个不同位移的相邻事件之间没有
   任何轴进出，桥面载荷恒定，等于前一个事件之后的载荷。
   - 只返回**位移长度大于零**且恒定载荷**严格大于核定载荷**的区段（等于核定值不算）；
   - **相邻且载荷相同**的区段先合并为一段；
   - 每项给出起始位移（含）、终止位移（不含）、该段恒定载荷，以及按
     **“位移差 × 1000 ÷ 车速”向上取整**的持续毫秒数
     （`ceil((end-start)×1000/speed)`）；
   - 同一位移处“进入先于离开”产生的**瞬时峰值继续参与第 4～5 条的原峰值裁决**，
     但它不占据任何正长度位移区间，**不进入区段、不计入累计时长**；
   - 全部区段毫秒数之和为累计超载时长；无超载区段时为空数组 `[]`、累计为 `0`。
   - 区段的起止位移与毫秒数均按**任意精度整数**计算，极大车辆跨度与低速（1mm/s）
     情形不溢出，毫秒数可能超出 int64，仍作为 JSON 数字精确返回。
...
| 字段 | 含义 |
| --- | --- |
| `max_load_kg` | 整个平移过程中落在有效桥面的轴载合计最大值，千克，任意精度整数 |
| `first_axle` / `last_axle` | 取得最大值时连续落桥轴区间的首尾轴序号（1 基，按提交顺序） |
| `displacement_mm` | 取得最大值时的车辆位移，毫米（车头轴抵桥入口为 0），任意精度整数——峰值位移超出 int64 时仍作为精确 JSON 数字返回 |
| `conclusion` | **`通行`**（最大值 ≤ 核定载荷）或 **`拦停`**（最大值 > 核定载荷） |

**省略 `speed_mm_per_s`（或填 `null`）时**，响应与上表完全一致、与引入车速能力前
**逐字节相同**，不会出现任何车速字段。

**提供 `speed_mm_per_s` 时**，在上述原峰值字段之后追加两个字段：

| 字段 | 含义 |
| --- | --- |
| `overload_segments` | 超载区段数组（按起始位移升序，无超载时为 `[]`）；每项含 `start_displacement_mm`（起始位移，含）、`end_displacement_mm`（终止位移，不含）、`load_kg`（该段恒定载荷）、`duration_ms`（向上取整毫秒数） |
| `total_overload_duration_ms` | 全部超载区段持续毫秒数之和；无超载区段时为 `0` |

```bash
# 桥长 3000、首尾轴距 2000，核定 11999，车速 1000mm/s：
# 三轴全落桥的区间 [2000,3000) 载荷恒定 12000，长度 1000mm，持续 ceil(1000×1000/1000)=1000ms。
curl -s -X POST localhost:8080/api/v1/bridge-window \
  -H 'Content-Type: application/json' \
  -d '{"axle_positions_mm":[0,1000,2000],"axle_loads_kg":[4000,4000,4000],"bridge_length_mm":3000,"approved_load_kg":11999,"speed_mm_per_s":1000}'
```
...
仅在某一位移的“进入后、离开前”瞬间超载（如桥长恰好容纳前后轴、12000 仅出现于
位移 3000 的瞬间）时，原峰值字段照常给出拦停结论，但
...
| 桥面窗口 | `POST /api/v1/bridge-window`：轴位置按车头方向严格递增且不重复（1～12 轴、位置/载荷逐项非 null、位置 ≥ 0、载荷 ≥ 1，位置与载荷均不设上限、可超 int64），桥长 1000～50000mm，核定载荷 1～200000kg；扫描全部进入/离开事件，返回最大桥面载荷、首尾轴、发生位移与 `通行`/`拦停` |
| 桥面窗口精度 | 位置、载荷、事件位移、落桥载荷合计全部按任意精度整数（big.Int）处理；超 int64 不解析失败、不溢出，`max_load_kg` 与 `displacement_mm` 均为精确 JSON 数字 |
